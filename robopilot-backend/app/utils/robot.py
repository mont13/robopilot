...
# Mock rtde modules for local development
class MockRTDE:
...
rtde_control = type("rtde_control", (), {"RTDEControlInterface": MockRTDE})()

rtde_io = type("rtde_io", (), {"RTDEIOInterface": MockRTDE})()

rtde_receive = type("rtde_receive", (), {"RTDEReceiveInterface": MockRTDE})()
...
METHODS_DEFINITION = """def unnamed():
  ...
  set_target_payload(1.100000, [-0.002000, 0.002000, 0.045000], [0.001804, 0.001804, 0.001804, 0.000000, 0.000000, 0.000000])
  ...
  def rq_gripper_enable_scan(enable, gripper_socket="1"):
      socket_set_var("MUTE", not enable,  gripper_socket)
      ack = socket_read_byte_list(3, gripper_socket)
      return is_ack(ack)
  end

  def rq_gripper_enable_tool(enable, gripper_socket="1"):
      socket_set_var("TOOL", enable,  gripper_socket)
      ack = socket_read_byte_list(3, gripper_socket)
      return is_ack(ack)
  end
  ...
  def rq_set_force(force, gripper_socket="1"):
      local force = floor(scale(force, [0, 255], [0.0, 255.0]))
      rq_set_var("FOR", force, gripper_socket)
  end

  def rq_set_speed(speed, gripper_socket="1"):
      local speed = floor(scale(speed, [0, 255], [0.0, 255.0]))
  ...
  def rq_set_pos(pos, gripper_socket="1"):
      local pos = floor(scale(pos, [0, 255], [0.0, 255.0]))
      rq_set_var("POS", pos, gripper_socket)
      rq_wait_for_pos_request(pos, gripper_socket)
  end
  ...
  def rq_set_pos_spd_for(pos, speed, force, gripper_socket="1"):
      ...
      rq_set_pos_spe_for_var(pos, speed, force, gripper_socket)
      rq_wait_pos_spe_for_request(pos, speed, force, gripper_socket)
  end
  ...
  def set_closed_norm(closed_norm, gripper_socket):
      gripper_closed_norm[to_num(gripper_socket) - 1] = closed_norm
  end

  def set_open_norm(open_norm, gripper_socket):
      gripper_open_norm[to_num(gripper_socket) - 1] = open_norm
  end

  def set_closed_mm(closed_mm, gripper_socket):
      gripper_closed_mm[to_num(gripper_socket) - 1] = closed_mm
  end

  def set_open_mm(open_mm, gripper_socket):
      gripper_open_mm[to_num(gripper_socket) - 1] = open_mm
  end
  ...
  # end: URCap Installation Node
  step_count_43e65a0d_3ff2_4d4b_ae6d_05c7bd2dcb9b = 0.0
  thread Step_Counter_Thread_d4d0d975_3536_4f4c_9c0d_e724773ef2e7():
    while (True):
      step_count_43e65a0d_3ff2_4d4b_ae6d_05c7bd2dcb9b = step_count_43e65a0d_3ff2_4d4b_ae6d_05c7bd2dcb9b + 1.0
      sync()
    end
  end
  run Step_Counter_Thread_d4d0d975_3536_4f4c_9c0d_e724773ef2e7()
"""
...
class Gripper:
    ...
    def __init__(self, ip_address: str, port: int = 30002):
        ...
        try:
            ...
            print(f"Connecting {self._ip_address}:{self._port}")
            ...
            print(f"Unexpected error during connecting to socket - {e}")
    ...
    def open_and_wait(self) -> None:
        ...
        self.socket.send(
            create_command('$ 3 "rq_open_and_wait()"\n   rq_open_and_wait()').encode(
                "utf8"
            )
        )
    ...
    def close_and_wait(self) -> None:
        ...
        self.socket.send(
            create_command('$ 3 "rq_close_and_wait()"\n   rq_close_and_wait()').encode(
                "utf8"
            )
        )
    ...
    def gripper_led_on(self) -> None:
        ...
        self.socket.send(
            create_command('$ 3 "rq_gripper_led_on()"\n   rq_gripper_led_on()').encode(
                "utf8"
            )
        )
    ...
    def move_and_wait_mm(self, distance: float) -> None:
        ...
        """
        if distance >= 0:
            print(f"Opening gripper to {distance} mm")
            self.socket.send(
                create_command(
                    '  $ 3 "rq_move_and_wait_mm('
                    + str(distance)
                    + ')"\n   rq_move_and_wait_mm('
                    + str(distance)
                    + ")"
...
class Robot:
    ...
    def __init__(
        self, ip: str = "192.168.0.94", initial_offset: list[float] | None = None
    ):
        """
    ...
    def connect(self):
        ...
        try:
            ...
            print(f"🔧 Mock connection to robot at {self.ip} (local dev mode).")
    ...
    def get_current_joints(self) -> list[float] | None:
        ...
        """
        if not self._connection_read:
            print("Error: Robot not connected.")
            return None

        # Use the existing connection instead of creating a new one
        try:
            actual_q = self._connection_read.getActualQ()
            return actual_q
        except Exception as e:
    ...
    def get_current_pose(self) -> list[float] | None:
        ...
        # Use the existing connection instead of creating a new one
        try:
            actual_tcp = self._connection_read.getActualTCPPose()
            return actual_tcp
        except Exception:
            return None
    ...
    @staticmethod
    def create_environment(robot_ip="192.168.0.94"):
        """
...
def run_pick_and_place_cycle(
    ...
    Args:
        ...
        start_sensor: Digital input pin for detecting start trigger
        end_sensor: Digital input pin for detecting object arrival
        conveyor_activate_pin: Digital output pin to activate conveyor
        conveyor_speed_pin: Analog output pin to control conveyor speed
        conveyor_speed: Speed value (0-1) for the conveyor
    """
    ...
    if robot.connect():
        ...
        # Connect and activate the gripper first
        if gripper.connect():
            try:
                print("🤖 Gripper connected")
                # Activate the gripper
                gripper.activate_and_wait()
                time.sleep(delay)
                print("🤖 Start pick and place cycle")

                # Move to home position
                robot.move_home()

                # Run conveyor until object is detected
                run_conveyor_monitor(
                    robot,
                    start_sensor=start_sensor,
                    end_sensor=end_sensor,
                    conveyor_activate_pin=conveyor_activate_pin,
                    conveyor_speed_pin=conveyor_speed_pin,
                ...
                robot.move_l(degree_to_rad(pick_position), speed=0.5, acceleration=0.03)
                ...
                # Always disconnect at the end, even if there was an error
