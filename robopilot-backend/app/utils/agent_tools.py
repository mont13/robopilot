...
def robot_move_home() -> dict:
    ...
    # Initialize robot
    try:
        my_robot = Robot(ip="192.168.0.94")
        print(my_robot)
    ...
    try:
        my_gripper = Gripper("192.168.0.94", 30002)
        time.sleep(DELAY)
    ...
    try:
        if not my_robot.connect():
            return {
                "success": False,
                "message": "Failed to connect to robot. Check if robot is powered on, network cable is connected, and IP address 192.168.0.94 is reachable.",
            }
...
def robot_reset_all() -> dict:
    ...
    try:
        ...
        errors.append(
            f"Robot reset failed: {str(e)}. Check if robot is powered and network accessible."
        )
    ...
    try:
        ...
        errors.append(
            f"Gripper reset failed: {str(e)}. Check gripper power and network connection."
        )
