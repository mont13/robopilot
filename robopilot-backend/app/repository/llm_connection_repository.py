...
from app.models.config import LLMConfig, LLMConnection, ProviderType
...
class LLMConnectionRepository(BaseRepository):
    ...
    async def get_connection_by_id(self, connection_id: str) -> LLMConnection | None:
        """
    ...
    async def update_connection(
        ...
        if need_activation:
            try:
                ...
                logging.error(f"Failed to set connection as active: {e!s}")
    ...
    async def delete_connection(self, connection_id: str) -> bool:
        ...
        if was_active:
            try:
                ...
                logging.error(
                    f"Failed to activate default connection after deletion: {e!s}"
                )

        return True

    async def get_active_connection(self) -> LLMConnection | None:
        """
    ...
    async def deactivate_all_connections_with_session(
        self, session, exclude_id: str | None = None
    ) -> None:
        """
        ...
        try:
            ...
            logging.error(f"Error deactivating connections: {e!s}")

    async def deactivate_all_connections(self, exclude_id: str | None = None) -> None:
        """
    ...
    async def activate_default_connection(self) -> LLMConnection | None:
        """
        ...
        async with get_db_session() as session:
            try:
                ...
                result = await session.execute(
                    ...
                    .where(LLMConnection.is_active == True)
                    .limit(1)
                )
    ...
    def connection_to_llm_config(self, connection: LLMConnection) -> LLMConfig:
        """
        Convert a connection entity to an LLM configuration for BeeAI framework.
        Only supports Ollama and OpenAI providers.
        """
