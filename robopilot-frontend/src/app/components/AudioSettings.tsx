...
const AudioSettings: React.FC<AudioSettingsProps> = ({
  ...
  const { selectedVoice, speakerId } = useAppSelector(selectAudioSettings);
  const { ttsEnabled, ttsAutoPlay } = useAppSelector(selectTtsSettings);
  const { streamingEnabled } = useAppSelector(selectStreamingSettings);
  const { connections, activeConnection, activateConnectionById } =
    useLMStudio();
  ...
  const loadVoices = async () => {
    ...
    try {
      ...
      if (!selectedVoice && voicesList.length > 0) {
        dispatch(setSelectedVoice(voicesList[0].id));
        dispatch(setTranscriptionLanguage("en")); // Default language
      }
      ...
      if (!activeConnection && connectionsList.length > 0) {
        ...
        console.log(
          "No active connection. Please select a connection from the dropdown.",
        );
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 sm:p-6 dark:text-gray-200">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    dispatch(setSelectedVoice(selectedVoiceId));
                    console.log(selectedVoiceId);

                    const languageCode = selectedVoiceId.split("_")[0];
                    console.log(languageCode);
                    dispatch(setTranscriptionLanguage(languageCode));
                  }}
                  disabled={isLoading}
                >
                  <option value="" disabled>
              ...
              <div>
                ...
                <div className="flex items-center">
                  <input
                    ...
                    value={speakerId}
                    onChange={(e) =>
                      dispatch(setSpeakerId(Number(e.target.value)))
                    }
          ...
          <div className="mb-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h5 className="text-md font-medium mb-1">Enable TTS</h5>
                  <p className="text-xs text-gray-500">
                    Allow text-to-speech playback for assistant messages
                  </p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    checked={ttsEnabled}
                    onChange={(e) => dispatch(setTtsEnabled(e.target.checked))}
                  />
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h5 className="text-md font-medium mb-1">Auto-play TTS</h5>
                  <p className="text-xs text-gray-500">
                    Automatically play TTS for new assistant messages in voice
                    mode
                  </p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    className="sr-only peer"
                    checked={ttsAutoPlay}
                    onChange={(e) => dispatch(setTtsAutoPlay(e.target.checked))}
                    disabled={!ttsEnabled}
                  />
                  <div
                    className={`w-11 h-6 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all shadow-sm ${ttsEnabled ? "bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 peer-checked:bg-blue-600" : "bg-gray-100 cursor-not-allowed"}`}
                  ></div>
                </label>
              </div>
            </div>
          ...
          <div className="mb-6">
            <h4 className="text-lg font-semibold mb-2">Response Settings</h4>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h5 className="text-md font-medium mb-1">Enable Streaming</h5>
                  <p className="text-xs text-gray-500">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      dispatch(setStreamingEnabled(e.target.checked))
                    }
                  />
                  <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600 shadow-sm"></div>
                </label>
